#ifndef DEVCON_WIN9X_MSVC6_SHIMS
#define DEVCON_WIN9X_MSVC6_SHIMS

/**
    These are shims to allow compiling and using of devcon in the
...
/* Not defined in the Win2000 DDK, so we can also exclude new
functionality, that definitely will not work. */
#ifndef _SETUPAPI_VER
    #define _SETUPAPI_VER       0
    #ifndef _WIN32_WINNT_WINXP
        #define _WIN32_WINNT_WINXP  1
    #endif
#endif

/* INF related error codes */
#define ERROR_NOT_AN_INSTALLED_OEM_INF 0xe000023c
#define ERROR_INF_IN_USE_BY_DEVICES    0xe000023d

/* Compensate for the lack of _count/_ARRAYSIZE at this point */
#define __ARRAYSIZE(X) (sizeof(X)/sizeof((X)[0]))

/* Array of strings */
typedef LPTSTR* PZPWSTR;

/* There is no ANSI version of this function, UuidFromString is not an alternative. */
static inline HRESULT WINAPI CLSIDFromStringA(LPSTR lpszString, LPCLSID lpCLSID)
{
    WCHAR szString[64];

    if(MultiByteToWideChar(CP_ACP, 0, lpszString, -1, szString, __ARRAYSIZE(szString)))
    {
        return CLSIDFromString(szString, lpCLSID);
    }

    return E_FAIL;
}
static inline HRESULT WINAPI CLSIDFromStringW(LPWSTR lpszString, LPCLSID lpCLSID)
{
    return CLSIDFromString(lpszString, lpCLSID);
}
#ifndef _UNICODE
    #define CLSIDFromString CLSIDFromStringA
#else
    #define CLSIDFromString CLSIDFromStringW
#endif

#endif  /* DEVCON_WIN9X_MSVC6_SHIMS */
